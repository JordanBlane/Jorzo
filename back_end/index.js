...
app.get('/users/add', (req,res)=>{
    ...
    const {user, pass} = req.query;
    //creates variable for query
    const INSERT_USER = `INSERT INTO userpass(username, password,bio,friends,messages,openchats,followers) VALUES('${user}', '${pass}', '', '','"example":{"message":[""]}','','')`;
...
//Login
app.get('/users/setlogin',(req,res)=>{
    const { username } = req.query;
    const SIGN_IN_USER = `UPDATE userpass SET signinstate = 'true' WHERE username = '${username}'`
    connection.query(SIGN_IN_USER, (err,result) => {
        if(err){return console.log(err)}
        return res.send('true')
    })
})
...
//Logout
app.get('/users/setlogout',(req,res)=>{
    const { username } = req.query;
    console.log(username + " SIGNING OUT")
    const SIGN_IN_USER = `UPDATE userpass SET signinstate = 'false' WHERE username = '${username}'`
    connection.query(SIGN_IN_USER, (err,result) => {
        if(err){return console.log(err)}
        return res.send('true')
    })
...
//add follower
app.get('/users/addfollower',(req,res)=>{
    const { friendname, username} = req.query;
    const ADD_FRIEND = `UPDATE userpass SET followers = CONCAT(followers, ';${friendname.trim()}') WHERE username='${username}'`

    //query to get all friends
    const GET_FRIENDS = `SELECT followers FROM userpass WHERE username="${username}"`;
    connection.query(GET_FRIENDS, (err, results) => {
        if(err){return console.log(err)}else{
            //gets array of all names
            let friendsArr = results[0].followers.split(";");
            for(let i =0;i<friendsArr.length;i++){
                if(friendsArr[i] == friendname.trim()){
                    //if friend matches name they want to add return false
                    console.log("MATCH")
...
//add to open chats
app.get('/users/addopenchat',(req,res)=>{
    const { username, friendname } = req.query;
    const ADD_FRIEND = `UPDATE userpass SET openchats = CONCAT(openchats, ';${friendname.trim()}') WHERE username='${username}'`

    //query to get all friends
    const GET_FRIENDS = `SELECT openchats FROM userpass WHERE username="${username}"`;
    connection.query(GET_FRIENDS, (err, results) => {
        if(err){return console.log(err)}else{
            //gets array of all names
            let friendsArr = results[0].openchats.split(";");
            for(let i =0;i<friendsArr.length;i++){
                if(friendsArr[i] == friendname.trim()){
                    //if friend matches name they want to add return false
                    console.log("MATCH")
